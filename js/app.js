// ============================================================
// GESTIONALE FINANZIARIO 2026 - App Logic v2
// ============================================================
...
function badgeCat(c) {
  const m={'Stipendi':'badge-amber','Affitto':'badge-blue','Tasse':'badge-red','Ufficio':'badge-gray','Extra':'badge-gray'};
...
function renderDashboard() {
  ...
  const incassato=totE;
  if (_chartTorta) _chartTorta.destroy();
  _chartTorta=new Chart(document.getElementById('chart-torta'),{
    type:'doughnut',
    data:{
      labels:['Incassato','Non pagato','Fattura in corso','Bando'],
      datasets:[{data:[incassato,nonPagato,fatturaInCorso,bando],backgroundColor:['#1B7B4B','#C0392B','#9A5F00','#1A4F8A'],borderWidth:2,borderColor:'#fff'}]
    },
    options:{responsive:true,plugins:{legend:{display:false}},cutout:'65%'}
  });
  document.getElementById('torta-legend').innerHTML=[
    ['#1B7B4B','Incassato',incassato],
    ['#C0392B','Non pagato',nonPagato],
    ['#9A5F00','Fattura in corso',fatturaInCorso],
    ['#1A4F8A','Bando',bando]
    ...
    <span style="font-weight:500;margin-left:auto">${fmt(v)}</span>
  </div>`).join('');
...
async function salvaEntrata() {
  ...
  const {error}=await sb.from('entrate').insert({user_id:currentUser.id,cliente,importo,
    mese:document.getElementById('e-mese').value,stato:document.getElementById('e-stato').value,
    note:document.getElementById('e-note').value});
  if (error){toast('Errore: '+error.message);return;}
  toast('Entrata salvata ✓'); closeModal('modal-entrata');
...
async function salvaUscita() {
  ...
  const {error}=await sb.from('uscite').insert({user_id:currentUser.id,
  ...
  if (error){toast('Errore: '+error.message);return;}
  toast('Uscita salvata ✓'); closeModal('modal-uscita');
...
function renderStipendi() {
  ...
  const grid={}; 
  _stipendi.filter(s=>s.tipo===tipo).forEach(s=>{
    const nome=s.dipendenti?.nome||'?';
    if (!grid[nome]) grid[nome]={};
    grid[nome][s.mese]=Number(s.importo);
  });
...
function renderScadenze() {
  ...
  let alertHtml='';
  if (scad.length) alertHtml+=`<div class="alert-box" style="border-color:var(--red);color:var(--red);background:var(--red-bg);margin-bottom:12px">⚠️ <strong>${scad.length} scadenze già passate</strong> — segna come Scaduto o registra il pagamento</div>`;
...
async function salvaScadenza() {
  ...
  const {error}=await sb.from('scadenze').insert({user_id:currentUser.id,
...
function openModal(id) {
  ...
  // Popola select clienti nello scadenziario
